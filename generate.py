...
def main():
    ...
    df = pl.DataFrame({"timestamp": timestamps}).with_columns(
        ...
        price=price,
        quantity=quantity,
       trade_price=pl.when(pl.col("type") == "trade")
                    .then(pl.col("price"))
                    .otherwise(None),
        trade_quantity=pl.when(pl.col("type") == "trade")
                    .then(pl.col("quantity"))
                    .otherwise(None),

        sides=sides,
        side=pl.when(pl.col("type") == "trade")
            .then(pl.col("sides"))
            .otherwise(None),
        ask_price=pl.when(pl.col("type") == "quote")
                    .then(pl.col("price") + 0.5)
                    .otherwise(None),
        ask_quantity=pl.when(pl.col("type") == "quote")
                    .then(pl.col("quantity"))
                    .otherwise(None),
        bid_price=pl.when(pl.col("type") == "quote")
                    .then(pl.col("price") - 0.5)
                    .otherwise(None),
        bid_quantity=pl.when(pl.col("type") == "quote")
                    .then(pl.col("quantity"))
                    .otherwise(None),
    )
